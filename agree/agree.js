window.onload = function () {
  ...
  /** 드래그 우클릭 방지를 위한 전역변수 선언 */
  var body = document.getElementsByTagName("body");

  /** 드래그 우클릭 방지 */
  //  onselectstart: 화면상에서 텍스트같은게 선택되는거
  //  ondragstart: 이미지 같은거 끌어보시면 마우스에 붙어서 딸려나오는거
  ...
  for (let i = 0; i < agree_check.length; i++) {
    agree_check[i].addEventListener("click", function () {
      if (
        agree_check[0].checked &&
        agree_check[1].checked &&
        agree_check[2].checked
      )
        agree_all.checked = true;
